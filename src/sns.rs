...
pub async fn subscribe(request: SubscribeRequest) -> Result<SubscribeResponse, SqsError> {
...
pub async fn get_subscription_attributes(
    ...
    if parts.len() > 1 {
        ...
        attributes.insert(
            "TopicArn".to_string(),
            "arn:aws:sns:local:000000000000:Topic".to_string(),
        );
    }

    attributes.insert(
        "SubscriptionArn".to_string(),
        _request.subscription_arn.clone(),
    );
